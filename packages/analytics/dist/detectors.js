...
async function detectAnomalies(pool, producer, route, currentP95, currentErrorCount, currentRequestCount) {
    ...
    for (const anomaly of anomaliesToEmit) {
        // Write to Postgres
        await pool.query(`INSERT INTO anomalies (detected_at, type, severity, route_template, details)
       VALUES ($1, $2, $3, $4, $5)`, [anomaly.detected_at, anomaly.type, anomaly.severity, anomaly.route_template, JSON.stringify(anomaly.details)]).catch(e => console.error('Failed to write anomaly to DB', e));
        // Publish to Kafka (fire and forget)
        producer.send({
            topic: 'anomalies.detected',
            messages: [{ key: anomaly.type, value: JSON.stringify(anomaly) }]
        }).catch((e) => console.error('Failed to publish anomaly to Kafka', e));
    }
}
